...
## Select keyframes from your own video (without an MLLM)

The standalone entry point accepts one local video and one query, calculates a
query-relevance score for every sampled candidate frame, runs AKS, and exports
the selected frames as JPG files. It does not load or call an MLLM.

```Shell
pip install -r requirements-keyframes.txt

python aks_keyframes.py \
  --video /path/to/video.mp4 \
  --query "the person opens the red suitcase" \
  --max-num-frames 32 \
  --output-dir ./aks_output/my_video
```

The output directory contains `frames/*.jpg` in chronological order and a
`manifest.json` with each keyframe's original frame index, timestamp, and
query-relevance score. Candidate frames are sampled once per second by default;
use `--sample-interval 0.5` for fast or fine-grained actions. Run
`python aks_keyframes.py --help` for all parameters.

The default `openai/clip-vit-base-patch32` checkpoint works best with English
queries. Translate a Chinese query to English for this checkpoint, or pass a
locally available compatible CLIP checkpoint with `--model-name`. The first run
downloads model weights from Hugging Face; subsequent runs use the local cache.

The command above is the previously provided standalone chain and remains
unchanged. For the new traceable path shared with `frame_select.py`, use:
...
`aks_keyframes_v2.py` and `frame_select.py` both call `aks_core.py`. The shared
core exposes `original` mode for repository quota reproduction and `robust`
mode for filling arbitrary frame budgets. See the detailed Chinese guide at
[docs/AKS_CUSTOM_USAGE.md](./docs/AKS_CUSTOM_USAGE.md).
