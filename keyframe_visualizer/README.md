...
页面分三列：左侧是算法切换和参数表单，中间是当前视频的 query 历史，右侧是结果区。左侧和中间两列做了收窄，把宽度让给右侧结果。
...
原视频、本次运行参数快照、耗时和帧数统计属于参考信息，收在结果下方的“相关信息”折叠区里，默认展开，视频播放器也限制了尺寸。窗口宽度小于 1240px 时两栏自动堆叠为上下排列。
